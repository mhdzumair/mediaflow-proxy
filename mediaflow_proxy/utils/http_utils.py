...
def get_proxy_headers(request: Request) -> ProxyRequestHeaders:
    ...
    request_headers.update(
        {k[2:].lower(): v for k, v in request.query_params.items() if k.lower().startswith("h_")}
    )
    ...
    response_headers = {
        ...
        if k.lower().startswith("r_")
    }

    # headers to remove from response
    x_headers_param = request.query_params.get("x_headers", "")
    remove_headers = [h.strip().lower() for h in x_headers_param.split(",") if h.strip()]
