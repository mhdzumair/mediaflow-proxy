...
async def request_with_retry(method: str, url: str, headers: dict, **kwargs) -> httpx.Response:
    ...
    Args:
        ...
        headers (dict): The headers to include in the request.
        **kwargs: Additional arguments to pass to the request. Supported special kwarg: 'verify' to control SSL verification.
    ...
    Raises:
        DownloadError: If the request fails after retries.
    """
    # Extract client-level kwargs (e.g., SSL verification) from kwargs
    client_kwargs = {}
    verify = kwargs.pop("verify", None)
    if verify is not None:
        client_kwargs["verify"] = verify

    # Create a client with specific settings if needed, otherwise use a default one.
    # This avoids creating a new client for every request unless necessary.
    if client_kwargs:
        async with create_httpx_client(**client_kwargs) as client:
