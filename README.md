...
#### Extract direct video stream from a supported host

```bash
curl "http://localhost:8888/extractor/video?host=Vavoo&redirect_stream=true&api_password=YOUR_PASSWORD&d=https://example.com/vavoo/stream/url"
```

This will extract and redirect to the direct video stream URL for the specified host (e.g., Vavoo). Replace `YOUR_PASSWORD` with your API password and the `d` parameter with the actual source URL.
