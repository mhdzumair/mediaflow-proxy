...
MediaFlow Proxy includes intelligent pre-buffering for both HLS and DASH streams, **disabled by default** to save resources. You can enable it via environment variables for improved streaming performance.
...
- `ENABLE_HLS_PREBUFFER`: Optional. Enables HLS pre-buffering for improved streaming performance. Default: `false`. Pre-buffering downloads upcoming segments ahead of playback to reduce buffering. Set to `true` to enable.
...
- `HLS_SEGMENT_CACHE_TTL`: Optional. TTL in seconds for cached HLS segments. Default: `300`.
...
- `ENABLE_DASH_PREBUFFER`: Optional. Enables DASH pre-buffering for improved streaming performance. Default: `false`. Pre-buffering downloads upcoming segments ahead of playback to reduce buffering. Set to `true` to enable.
